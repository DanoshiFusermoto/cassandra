...
public class Bloom
{
    ...
    private final ThreadLocal<MessageDigest> digester = ThreadLocal.withInitial(() -> getDigester(hashName, null));
    ...
    private int[] createHashes(byte[] data, int hashes) 
    {
        ...
        {
            byte[] digest;
            this.digester.get().update(salt);
            salt++;
            digest = this.digester.get().digest(data);                
